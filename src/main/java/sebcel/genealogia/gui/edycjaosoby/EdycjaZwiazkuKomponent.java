...
public class EdycjaZwiazkuKomponent extends EdycjaKomponent {
	...
	private void odswiezListy() {
		List<OsobaStruct> mezczyzni = DatabaseDelegate.getMezczyzni();
		List<OsobaStruct> kobiety = DatabaseDelegate.getKobiety();
		...
		for (OsobaStruct osoba : mezczyzni) {
			tMezczyzna.addItem(osoba);
		}
		for (OsobaStruct osoba : kobiety) {
			tKobieta.addItem(osoba);
		}
	}
	
	public boolean zapiszDane() {
		if (trybPracy==TrybPracy.DODAWANIE) daneZwiazku = new DaneEdycjiZwiazkuStruct();
		...
		daneZwiazku.mezczyzna = (OsobaStruct) tMezczyzna.getSelectedItem();
		daneZwiazku.kobieta = (OsobaStruct) tKobieta.getSelectedItem();
	...
	public DiagramInfoStruct getDiagramInfo(Long id) {
		...
		diagramInfo.idKorzenia = daneZwiazku.mezczyzna.id;
		diagramInfo.nazwa = daneZwiazku.mezczyzna.opis+" i "+daneZwiazku.kobieta.opis;
		diagramInfo.opis = "Diagram zwi¹zku "+daneZwiazku.mezczyzna.opis+" "+daneZwiazku.kobieta.opis;
